...
The primary development target is the **Waveshare ESP32-S3-LCD-1.47**. The **TTGO T-Display V1.1** is also a supported target for the physically validated shared-source Portable Dashboard and the display, Wi-Fi, MQTT, and NTP demos listed below. The complete Waveshare Home Dashboard is not TTGO-supported.
...
Physically validated Portable Dashboard plus display and service demos.
...
`app_portable_dashboard` and `ttgo_portable_dashboard` compile the same application and page source from `src/apps/portable_dashboard/`. Waveshare and TTGO also share the reusable `Display` and `Display_Widgets` implementations. Shared pages select layout geometry from `Display::width()` and `Display::height()` rather than using board-specific widgets. On narrow screens, prefer shorter labels and tighter gaps before reducing widget or text size.
...
The second application is shared unchanged by Waveshare and TTGO:

```text
src/apps/portable_dashboard/
```

Build it with `app_portable_dashboard` for Waveshare or `ttgo_portable_dashboard` for TTGO. Physical testing has passed on both boards using the same application and page source, with no TTGO-specific application or widget fork.

Its Device, Clock, and MQTT pages rotate every 10 seconds. The Device page redraws for meaningful network, MQTT, or RSSI changes; the Clock page draws once per activation; and the MQTT page redraws on activation and when a message arrives while active. It uses `NetworkService`, `MQTTService`, `TimeService`, `Display`, `Display_GFX`, `Display_Widgets`, and `Logger`.

The Portable Dashboard intentionally excludes the Waveshare-specific SD, RGB LED, button, PNG, and native-USB features owned by the full Home Dashboard.
