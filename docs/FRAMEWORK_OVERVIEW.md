...
```text
Home Dashboard / Portable Dashboard / Demos
        │
        ├── Page modules
        ├── NetworkService
        ├── MQTTService
...
The standalone MQTT demo uses `NetworkService` and `MQTTService`, including on
the physically validated TTGO environment.
...
The Portable Dashboard is the application-level portability proof. Its
`app_portable_dashboard` and `ttgo_portable_dashboard` environments compile
the exact same source and have been physically validated on Waveshare and
TTGO. It combines Device, Clock, and MQTT pages with `NetworkService`,
`MQTTService`, `TimeService`, and the shared display/UI stack without a
TTGO-specific application or widget fork.

The Portable Dashboard deliberately omits Waveshare-only SD, RGB LED, button,
PNG, and native-USB functionality. This validation does not imply that the
complete Home Dashboard works on TTGO.

## Home Dashboard Ownership

The full Home Dashboard remains owned by and physically validated for the
Waveshare target. Its hardware-specific features are outside current TTGO
support.
