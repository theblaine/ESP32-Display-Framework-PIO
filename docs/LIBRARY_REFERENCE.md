...
The current standalone `demo_mqtt`/`ttgo_mqtt` source uses `NetworkService` and
`MQTTService`. Those demos validate the shared services plus MQTT and JSON
behavior on both board targets.
